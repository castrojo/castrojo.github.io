...
Spinning up on AWS tools can be tough if you’re new and not used to the
tools. I quickly found myself misusing juju when I needed an instance
real quick. I just fire up a bootstrap node and then ssh into it.

    juju bootstrap
    juju ssh/0 

My brain tends to remember this much better than any other method for
some reason. Then I just `juju destroy-environment` when I’m done.
