...
My friend Adam Lincoln has been working on a Python implementation of
[Pal](http://palcal.sourceforge.net/), with one key feature, Google
Calendar support.

Finally, a companion to mutt for CLI people. The project is called
[pygpal](https://edge.launchpad.net/pygpal), and it’s hosted on
launchpad. It has some great features like seperate colors for each
calendar, add/delete/edit of calendars, with full sync to Google
Calendar.

This was a side project for Adam so he could play with the gcal python
API. If it’s useful, please consider contributing to the project,
specifically the “first run” experience needs some work. Feel free to
branch it and run with the ball.
