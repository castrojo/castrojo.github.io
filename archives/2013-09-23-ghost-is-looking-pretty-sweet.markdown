...
A friend of mine let me play around with [Ghost](http://ghost.org), the
new simplified blogging engine.

![](http://www.jorgecastro.org/images/ghost.png)

It’s still missing some features, but for me it’s about 90% there. The
Markdown editor is excellent, it’s really all about no-frills content
and publishing, which I like. There’s no content importing from
Octopress (yet), so I’m going to not move (yet); but when it’s out for
the public it should be a nice alternative for those of us who prefer
simple blogging. And I must admit after hosting a static blog for so
cheap it gets kind of weird moving back to something that needs a
working web server (First world problems I know.)

I think it would be nice to have a hosted service like this that would
just let me dump the rendered html into an S3 bucket.

Also check out Lee Hutchinson’s [review of
Ghost](http://arstechnica.com/gadgets/2013/09/fancy-node-js-based-blogging-app-ghost-goes-live-to-backers/)
on Ars Technica.
