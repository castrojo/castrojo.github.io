...
The [Juju website](http://juju.ubuntu.com) was looking a bit old school
with it’s Moin wikiness. So we updated it, using the same layout as
[developer.ubuntu.com](http://developer.ubuntu.com). This will let us
have fresher content and be able to quickly update the site with the
latest videos and tutorials from our various charm schools. We still
need to integrate [jujucharms.com](http://jujucharms.com) and the
documentation, so there’s still some work to do.

Feel free to [file a
bug](https://bugs.launchpad.net/juju-website/+filebug) if you see
something that needs fixing. The theme content is [on
Launchpad](https://launchpad.net/juju-website) if you want to take a
look.
