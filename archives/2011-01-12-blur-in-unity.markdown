...
Mirco Müller and Jay Taoko have just landed a Gaussian blur for
quicklists in Unity trunk. It uses the OpenGL GLSL shading language on
Nvidia. Intel and AMD/ATI will be implemented over the next coming
weeks.

(AMD and Intel are currently using ARBprograms - later on if the
hardware supports it they will also use OpenGL GLSL shading language)
